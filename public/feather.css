(function () {
	...
	// c3 falls back to a detached node when `bindto` matches nothing, which
	// leaves an orphan chart and a live MutationObserver per missing container.
	// Only build the charts whose wrapper is actually on the page.
	function generateChart(config) {
		if (!config || !document.querySelector(config.bindto)) {
			return null;
		}
